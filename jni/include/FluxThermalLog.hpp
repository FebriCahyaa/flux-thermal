...
inline void init(const std::string &log_path = LOG_FILE) {
    try {
        ...
        fprintf(stderr, "FluxThermalLog init failed: %s\n", ex.what());
        exit(EXIT_FAILURE);
    }
}
...
inline void set_log_level(int level) {
    ...
    LOGI_TAG("FluxThermalLog", "Log level changed to {}", spdlog::level::to_string_view(spdlog_level));
}
